...
func _on_no_return_body_entered(body: Node2D) -> void:
	...
	if global.player_past_entry == true:
		...
		DialogueManager.show_example_dialogue_balloon(load("res://crash_site_text.dialogue"), "no_escape")


func _on_die_here_body_entered(body: Node2D) -> void:
	if global.player_past_entry == true:
		global.cutscene = true
		DialogueManager.show_example_dialogue_balloon(load("res://crash_site_text.dialogue"), "die_here")
